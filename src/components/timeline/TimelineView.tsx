...
export default function TimelineView({ events: allEventsFromProps, onDeleteEvent }: TimelineViewProps) {
  ...
  const handleDayClick = (day: Date | undefined) => {
    if (day) {
      ...
      if (eventsOnDay.length > 0) {
        ...
        // Optionally, provide feedback if no events on clicked day
        toast({ title: "No Events", description: `No events scheduled for ${format(day, 'MMMM d, yyyy')}.` });
        setIsModalOpen(false); // Ensure modal is closed if no events
      }
    }
  };
  ...
  const handleDeleteEventFromModal = (eventId: string) => {
    if (onDeleteEvent) {
      ...
      // Check if there are any events left for the selected day
      const remainingEvents = eventsForSelectedDay.filter(e => e.id !== eventId);
      if (remainingEvents.length === 0) {
        setIsModalOpen(false); // Close modal if no events are left
      }
      // The eventsForSelectedDay will update via useMemo when allEventsFromProps changes
    }
  };
      ...
      {selectedDate && (
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          <DialogContent className="frosted-glass sm:max-w-lg max-h-[80vh]">
            ...
            {eventsForSelectedDay.length > 0 && (
              <ScrollArea className="mt-4 pr-2 -mr-2 max-h-[calc(80vh-12rem)]"> {/* Adjusted max-h */}
