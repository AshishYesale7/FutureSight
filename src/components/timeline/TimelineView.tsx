...
import { ChevronLeft, ChevronRight, CalendarDays, Bot, Trash2, Clock } from 'lucide-react'; // Added Clock
import { format, isToday, addMonths, subMonths, parseISO, compareAsc, getYear, getMonth, startOfDay } from 'date-fns';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Dialog, DialogTrigger, DialogContent, DialogHeader, DialogTitle, DialogDescription } from '@/components/ui/dialog'; // Added DialogDescription
...
// Helper to check if a Date object's time is midnight
const isMidnight = (date: Date): boolean => {
  return date.getHours() === 0 && date.getMinutes() === 0 && date.getSeconds() === 0 && date.getMilliseconds() === 0;
};

interface GroupedEvent {
  date: Date; // This is the start of the day for grouping purposes
...
export default function TimelineView({ events: allEventsFromProps, onDeleteEvent }: TimelineViewProps) {
  ...
  const handleDelete = () => {
    if (selectedEvent && selectedEvent.isDeletable && onDeleteEvent) {
      onDeleteEvent(selectedEvent.id);
      toast({ title: "Event Deleted", description: `"${selectedEvent.title}" has been removed from your timeline.` });
  ...
  const processedEvents = useMemo(() => {
    return allEventsFromProps
      .map(e => ({ ...e, date: e.date instanceof Date && !isNaN(e.date.valueOf()) ? e.date : parseISO(e.date as unknown as string) })) // Ensure date is a Date object
  ...
  const filteredAndGroupedEvents = useMemo(() => {
    ...
    return processedEvents
      ...
      .reduce((acc, event) => {
        const dayKey = format(startOfDay(event.date), 'yyyy-MM-dd'); // Group by the start of the day
        if (!acc[dayKey]) {
          acc[dayKey] = { date: startOfDay(event.date), events: [] };
        }
  ...
  return (
    <Card className="frosted-glass w-full flex flex-col shadow-xl overflow-hidden">
      ...
      <CardContent className="p-0 flex-1">
        <ScrollArea className="w-full h-full" type="auto">
          <div className="relative p-6">
            ...
            {sortedEventGroups.map(({ date: groupDate, events: dayEvents }) => ( // groupDate is startOfDay
              <div key={format(groupDate, 'yyyy-MM-dd')} className="relative flex items-start mb-8 last:mb-0">
                <div className="sticky top-4 z-10 flex-shrink-0 w-16 flex flex-col items-center mr-4">
                  <div className={cn(
                      ...
                      isToday(groupDate) ? 'bg-accent ring-2 ring-accent/70' : getEventTypeDotColor(dayEvents[0].type)
                    )}>
                    {isToday(groupDate) && <div className="w-2.5 h-2.5 bg-white rounded-full"></div>}
                  </div>
                  <div className="font-bold text-primary text-lg leading-tight">{format(groupDate, 'dd')}</div>
                  <div className="text-xs text-muted-foreground leading-tight">{format(groupDate, 'MMM')}</div>
                   {isToday(groupDate) && <Badge variant="default" className="mt-1 text-xs px-1.5 py-0.5 bg-accent text-accent-foreground">Today</Badge>}
                </div>
                           ...
                           <DialogHeader className="space-y-1">
                            ...
                            <div className="flex justify-between items-center pt-1">
                                ...
                                {selectedEvent.isDeletable && (
                                  <Button variant="outline" size="sm" className="text-xs border-destructive/50 text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={handleDelete}>
